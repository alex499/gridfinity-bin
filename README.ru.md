...
- Перегородки по свободной сетке: `rows` задаёт число ячеек в каждом ряду, и ряды могут
  ...
  ними, и сами ничего не несут — ради этого и был сделан переход на крышку. Те, что делят
...
карточка от 1x1 не подойдёт к 2x2. Крышка привязана только к размеру, но не к раскладке —
она закрывает всё независимо от того, как расставлены ячейки.
