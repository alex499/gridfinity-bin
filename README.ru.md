...
[открыть его в конфигураторе](https://alex499.github.io/gridfinity-bin/#units_x=1&units_y=1&units_z=3&closure=auto&split_lid=0&scoop=1&wall_thickness=1&floor_thickness=1&card_length=15&scoop_flush=0&solid_foot=0&cells=4&view=bin).
...
- Этим пазом пользуются две пластины, и выбирает между ними `closure`. Одна — накладная
  ...
  содержимому переходить из ячейки в ячейку, — но любую можно затребовать напрямую, а
  `none` оставляет бин открытым, вообще без паза
...
| `closure` | auto | auto, card, lid, none | Под какую пластину резать паз; `auto` — карточка, пока нет перегородок, дальше крышка, а `none` оставляет бин открытым, без паза |
